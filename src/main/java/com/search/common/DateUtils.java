...
public class DateUtils {
    ...
    public static String getDateToMinute(String fullDateStr) {
        return fullDateStr == null ? null
                : (fullDateStr.length() >= dateLength ? fullDateStr.substring(0, dateLength)
                        : fullDateStr);
    }

    /**
     * 返回指定年度的所有周。List中包含的是String[2]对象 string[0]本周的开始日期,string[1]是本周的结束日期。 日期的格式为YYYY-MM-DD
     * 每年的第一个周，必须包含星期一且是完整的七天。 例如：2009年的第一个周开始日期为2009-01-05，结束日期为2009-01-11。
     * 星期一在哪一年，那么包含这个星期的周就是哪一年的周。 例如：2008-12-29是星期一，2009-01-04是星期日，哪么这个周就是2008年度的最后一个周。
     *
     ...
     * @throws ParseException @
     */
    public static List<String> getWeekDaysAroundDate(String refDate, int weekOffset,
